/**
 * @file breakdown-chords.js
 * @description Chords branch of the post-answer breakdown panel for The Sound Travels
 * Ear Training. Handles all four chord families — polychords, UST, slash, and regular
 * chords — and owns the complete voice leading and resolution rendering pipeline.
 *
 * Responsibilities:
 *   - Lookup tables:  `RESOLUTION_TARGETS`, `VL_INTERVAL_NAMES`
 *   - VL helpers:     `vlRoleLabel()`, `buildResolutionMidi()`, `getResolutionInfo()`,
 *                     `computeVoiceLeading()`, `makeVoiceLeadingRow()`
 *   - Theory helpers: `computeRiemannRelations()`, `computeTritoneSubInfo()`,
 *                     `computeDimEnharmonics()`, `computeDimDomSubs()`,
 *                     `computeAugEnharmonics()`, `computeHalfDimContext()`,
 *                     `computeSusResolution()`
 *   - DOM builders:   `makeRiemannRow()`, `figuredBass()`, `nameChordFromIntervals()`
 *   - Renderer:       `showBreakdownChords()`
 *
 * Cross-cutting resolution state (`resolutionActive`, `selectedResolution`,
 * `playResolution`, etc.) lives in `breakdown.js` — those symbols are shared
 * with the audio and notation paths.
 *
 * Dependencies (globals from earlier layers):
 *   `makeNameHeader`, `makeBDRow`, `makeCSGroup`, `makeChordScalesRow`,
 *   `joinSep`, `intervalAbbr`, `semitonesToNumeral`, `qualityFullName`,
 *   `makePill`, `SEMITONE_TO_ROMAN`, `INTERVAL_ABBR`,
...
/**
 * Fallback resolution table used when `analyseChord()` (`voiceLeading.js`) has not
 * yet run or returns no result. Each entry maps a chord symbol to a single canonical
 * resolution target defined by a semitone offset from the chord root.
 *
 * Entry shape: `{ offset: number, quality: string, label: string }`
 *   - `offset`  — semitones UP from chord root to resolution root
 *   - `quality` — target chord quality key for `buildResolutionMidi()`
 *   - `label`   — display string shown in the Resolve button and breakdown
 *
 * Theory notes:
 * @type {Object.<string, { offset: number, quality: string, label: string }>}
 */
// Theory notes (retained for algorithmic reference):
...
/**
 * Semitone count → interval name string for voice leading move labels (ascending).
 * Used by `computeVoiceLeading()` and `buildVLTable()` to label each voice's motion.
 *
...
/**
 * Returns the harmonic role label for a source note, based on its semitone distance
 * above the chord root. Used in the voice leading table to annotate each departing voice.
 *
 * @param {number} semiFromRoot - Semitone interval from the chord root (mod 12 applied internally).
 * @returns {string} Role label (e.g. `'root'`, `'3rd'`, `'♭7th'`), or `''` if unmapped.
 */
...
/**
 * Builds a close-position MIDI note array for a resolution target chord.
 * Used by `getResolutionInfo()` and the voice leading fallback path when
 * pre-computed voice leading moves are unavailable.
 *
 * @param {number} targetRootMidi - MIDI note number of the target chord root.
 * @param {string} quality - Target chord quality: `'maj'` | `'min'` | `'dom7'` | `'maj7'` | `'m7'`.
 * @returns {number[]} Sorted array of MIDI note numbers for the target chord.
 */
...
/**
 * Returns the resolution target for the currently displayed chord. Consults sources
 * in priority order: (1) user-selected resolution from the Voice Leading panel,
 * (2) family-specific logic for poly / UST / slash chords, (3) the primary context
 * from `currentVoiceLeadingAnalysis` (Pass 1 engine), (4) `RESOLUTION_TARGETS`
 * fallback table. Returns `null` if no valid target can be determined.
 *
 * @returns {{ targetRootMidi: number, targetMidi: number[], targetName: string,
 *             targetQuality: string, label: string, targetSymbol?: string } | null}
 */
...
/**
 * Computes voice leading moves from a source chord to a target chord. Uses the
 * rule-based engine (`computeVoiceLeadingRules()` from `voiceLeading.js`) when a
 * harmonic context is cached; falls back to a proximity search otherwise.
 *
 * @param {number[]} sourceMidi - MIDI notes of the source chord.
 * @param {number[]} targetMidi - MIDI notes of the target chord.
 * @returns {Array<{ fromName: string, toName: string, dir: string, absSemi: number,
 *                   intervalName: string, role: string, isCommonTone: boolean }>}
 */
 ...
 *
 * When `currentVoiceLeadingAnalysis` is populated and non-ambiguous, renders one
 * collapsible `cs-section` per harmonic context, each containing:
 ...
 *
 * Falls back to a single-resolution display for ambiguous chord families (aug, sus,
 * poly, UST, slash) or when the analysis cache is unavailable.
 *
 * Mobile and desktop paths are structurally separate: mobile renders a full-width
 * stack; desktop renders a `breakdown-row` with the label on the left.
 *
 * @param {HTMLElement} panel - The element to render the voice leading section into.
 */
...
/**
 * Computes the four Riemannian neo-tonal transformations for a major or minor triad.
 * Each transformation produces a related chord by moving one or two voices by a semitone
 * while flipping the chord quality (major ↔ minor).
 *
 * @param {number} rootPc - Root pitch class of the chord (0–11).
 * @param {string} quality - Chord quality: `'major'` | `'minor'`.
 * @param {string} sym - Chord symbol, used for enharmonic note spelling.
 * @returns {{ R: Object, L: Object, P: Object, N: Object }} Each value is
 *   `{ letter, full, desc, chord }` where `chord` is the display name (e.g. `'Am'`).
 */
